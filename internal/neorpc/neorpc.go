...
// nrpc client
func GetClient(conn string) (*nrpc.Client, func(), error) {
	client, err := nrpc.NewClient(conn, 1000)
	if err != nil {
...
func FindBestoneForBackup(self string) (string, error) {
	// TODO: 获取ReplMode的RPC
	var replMode model.MysqlReplMode = ""
	replMode = model.ReplModeMGR //====
		...
		if node != leader {
			if rsp, err := GetMysqlStatusRPC(node); err == nil {
				...
				if replMode == model.ReplModeMGR {
					if num, _ := strconv.Atoi(GTID.Txns_Behind_Master); num < 100 &&
						node != self {
...
func RequestBackupRPC(fromnode string, conf *config.MysqlConfig, backupdir string) (*model.BackupRPCResponse, error) {
...
func MysqlResetSlaveRPC(node string) (*model.MysqlRPCResponse, error) {
	cli, cleanup, err := GetClient(node)
	if err != nil {
		return nil, err
	}
