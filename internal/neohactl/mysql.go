...
func mysqlStopMonitorCommandFn(cmd *cobra.Command, args []string) {
	...
	conf, err := GetConfig()
	ErrorOK(err)
	rsp, err := neorpc.StopMonitorRPC(conf.Endpoint)
	ErrorOK(err)
	RspOK(rsp.RetCode)
...
func mysqlStartMonitorCommandFn(cmd *cobra.Command, args []string) {
	...
	conf, err := GetConfig()
	ErrorOK(err)
	rsp, err := neorpc.StartMonitorRPC(conf.Endpoint)
	ErrorOK(err)
	RspOK(rsp.RetCode)
...
func mysqlStartCommandFn(cmd *cobra.Command, args []string) {
	...
	conf, err := GetConfig()
	ErrorOK(err)
	neorpc.StartMysqldRPC(conf.Endpoint)
...
func mysqlShutDownCommandFn(cmd *cobra.Command, args []string) {
	...
	// shutdown
	neorpc.ShutdownMysqldRPC(conf.Endpoint)

	// wait mysqld shutdown
	neorpc.WaitMysqldShutdownRPC(conf.Endpoint)
...
func getLocalTrxCount(self string, bestone string) (int, error) {
	...
	rsp1, err := neorpc.GetGTIDRPC(bestone)
	if err != nil {
	...
	rsp1, err = neorpc.GetGTIDRPC(self)
	if err != nil {
	...
	rsp2, err := neorpc.GetGTIDSubtractRPC(self, localGTID.Executed_GTID_Set, fromGTID.Executed_GTID_Set)
	if err != nil {
...
func mysqlRebuildMeCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	bestone := ""
	version := strings.TrimSpace(conf.Database.Mysql.Version)
	datadir := conf.Database.Mysql.Backup.BackupDir
	binlogDir := ""
	binlogPrefix := ""
	maxAllowedLocalTrxCount := conf.Database.Mysql.Backup.MaxAllowedLocalTrxCount

	// 1. first to check if I am leader or MGR master
	{
		log.Warning("S1-->check.raft.leader")
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		if leader == self {
			log.Panic("I[%v].am.leader.you.can.not.rebuildme.sir", self)
		}
		log.Warning("S1-->check.MGR.master")
		status, err := neorpc.GetMysqlStatusRPC(self)
		if err == nil && status.RetCode == model.OK && status.MGRStatus.Role == model.MGRRolePrimary {
			log.Panic("[%v].I.am.MGR.leader.you.can.not.rebuildme.sir", self)
		}
	}
	...
	// 2. find the best to backup and check gtid
	{
		if fromStr != "" {
			...
			bestone, err = neorpc.FindBestoneForBackup(self)
			ErrorOK(err)
		}
		...
		if force {
			...
			working, err := neorpc.MysqlIsWorkingRPC(self)
			ErrorOK(err)
			if working == false {
	...
	{
		rsp, err := neorpc.GetMysqldStatusRPC(bestone)
		ErrorOK(err)
	...
	// 4. set learner
	{
		...
		if _, err := neorpc.SetLearnerRPC(self); err != nil {
	...
	// 5. stop monitor
	{
		...
		neorpc.StopMonitorRPC(self)
	}

	// 6. force kill mysqld
	{
		log.Warning("S6-->kill.mysql")
		err := neorpc.KillMysqldRPC(self)
		ErrorOK(err)

		// wait
		err = neorpc.WaitMysqldShutdownRPC(self)
		ErrorOK(err)

		// set the mysql state to dead, avoid failure to rebuild node with small amounts of data
		err = neorpc.SetMysqlStateRPC(self, model.MysqlDead)
		ErrorOK(err)
	}
	...
	// 8. remove data files
	{
		...
		*/
		args = []string{
			"-c",
			fmt.Sprintf("grep 'log-bin=' %s | sed -r '/^#/d' | awk -F '=' '{print $2}' | tail -n 1", conf.Database.Mysql.DefaultsFile),
		}
		...
		args = []string{
			"-c",
			fmt.Sprintf("grep 'log-bin-index=' %s | sed -r '/^#/d' | awk -F '=' '{print $2}' | tail -n 1", conf.Database.Mysql.DefaultsFile),
		}
	...
	// 9. do backup from bestone
	{
		...
		rsp, err := neorpc.RequestBackupRPC(bestone, conf.Database.Mysql, datadir)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	...
	// 10. do apply-log
	{
		...
		err := neorpc.DoApplyLogRPC(conf.Endpoint, datadir)
		ErrorOK(err)
	...
	// 11. start mysqld
	{
		...
		if _, err := neorpc.StartMonitorRPC(self); err != nil {
	...
	// 12. wait mysqld running
	{
		...
		neorpc.WaitMysqldRunningRPC(self)
	...
	// 13. wait mysql working
	{
		...
		neorpc.WaitMysqlWorkingRPC(self)
	...
	// 14. stop slave and reset slave
	{
		...
		if conf.Database.Mysql.ReplMode == model.ReplModeSemiSync {
			if _, err := neorpc.MysqlResetSlaveAllRPC(self); err != nil {
				log.Error("mysql.stop.adn.reset.slave.error[%v]", err)
			}
		} else {
			version := strings.TrimSpace(conf.Database.Mysql.Version)
			if version == "mysql80" {
				neorpc.MysqlResetSlaveRPC(self) // for MGR
			}
		}
	...
	// 15. set gtid_purged
	{
		...
		if version == "mysql80" {
			...
			neorpc.MysqlResetMasterRPC(self)
			log.Warning("S15-->reset.master.end....")

			gtid, err := neorpc.GetXtrabackupGTIDPurged(self, datadir)
			ErrorOK(err)

			log.Warning("S15-->set.gtid_purged[%v].begin....", gtid)
			rsp, err := neorpc.SetGlobalVarRPC(self, fmt.Sprintf("SET GLOBAL gtid_purged='%s'", gtid))
			ErrorOK(err)
			RspOK(rsp.RetCode)
	...
	{
		log.Warning("S17-->wait[%v ms].change.to.master...", conf.Election.Raft.ElectionTimeout)
	...
	if conf.Database.Mysql.ReplMode == model.ReplModeSemiSync {
		...
		if _, err := neorpc.MysqlStartSlaveRPC(self); err != nil {
...
func mysqlDoBackupCommandFn(cmd *cobra.Command, args []string) {
	...
	{
		node, err := neorpc.FindBestoneForBackup(self)
		ErrorOK(err)
		bestone = node
	...
	// 3. do backup from bestone
	{
		...
		rsp, err := neorpc.RequestBackupRPC(bestone, conf.Database.Mysql, backupdir)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	...
	// 4. do apply-log
	{
		...
		err := neorpc.DoApplyLogRPC(conf.Endpoint, backupdir)
		ErrorOK(err)
...
func mysqlCancelBackupCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		...
		rsp, err := neorpc.BackupCancelRPC(self)
		ErrorOK(err)
		RspOK(rsp.RetCode)
...
func mysqlCreateUserCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		rsp, err := neorpc.CreateNormalUserRPC(leader, user, host, passwd, ssl)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	}
...
func mysqlCreateSuperUserCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		rsp, err := neorpc.CreateSuperUserRPC(leader, user, host, passwd, ssl)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	}
...
func mysqlDropUserCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		rsp, err := neorpc.DropUserRPC(leader, user, host)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	}
...
func mysqlChangePasswordCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		...
		rsp, err := neorpc.ChangeUserPasswordRPC(leader, user, host, passwd)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	}
...
func mysqlSetGlobalVarCommandFn(cmd *cobra.Command, args []string) {
	...
	conf, err := GetConfig()
	ErrorOK(err)
	rsp, err := neorpc.SetGlobalVarRPC(conf.Endpoint, args[0])
	ErrorOK(err)
	RspOK(rsp.RetCode)
...
func mysqlKillCommandFn(cmd *cobra.Command, args []string) {
	...
	conf, err := GetConfig()
	ErrorOK(err)
	err = neorpc.KillMysqldRPC(conf.Endpoint)
	ErrorOK(err)
...
func mysqlStatusCommandFn(cmd *cobra.Command, args []string) {
	...
	ErrorOK(err)
	// mysqld info
	if running, err := neorpc.MysqldIsRunningRPC(self); err == nil {
		status.Mysqldrunning = running
		// slave info
		if running {
			rsp, err := neorpc.GetGTIDRPC(self)
			ErrorOK(err)
			...
			mysqlworking, err := neorpc.MysqlIsWorkingRPC(self)
			ErrorOK(err)
			status.Mysqlworking = mysqlworking
		}
	}

	if rsp, err := neorpc.GetMysqldStatusRPC(self); err == nil {
...
func mysqlCreateUserWithPrivilegesCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		rsp, err := neorpc.CreateUserWithPrivRPC(leader, grantUser, grantPasswd, grantDatabase, grantTable, grantHost, grantPrivs, requireSSL)
		ErrorOK(err)
		RspOK(rsp.RetCode)
	}
...
func mysqlGetUserCommandFn(cmd *cobra.Command, args []string) {
	...
	self := conf.Endpoint
	{
		leader, err := neorpc.GetClusterLeader(self)
		ErrorOK(err)
		rsp, err := neorpc.GetMysqlUserRPC(leader)
		ErrorOK(err)
		RspOK(rsp.RetCode)
