...
const (
	...
	RAFTMYSQL_WAITAPPLYRELAYLOG  RAFTMYSQL_STATUS = "WaitApplyRelayLog"
...
type Raft struct {
	...
	// The state of change master(includes Follower's and Leader's)
	CmtState string

	// UUID, only candidate need
	UUID string
...
func (req *RaftRPCRequest) GetUUID() string {
	return req.Raft.UUID
}
...
type RaftStats struct {
	...
	// namely do not got majority hb-ack
	LessHearbeatAcks uint64

	// How many times the leader do not got majority hb-ack for finish change master to
	LessCmtHearbeatAcks uint64
