...
func TestMysqlBaseGetMGRGTID(t *testing.T) {
	// 1. mysql up
	{
		db, mock, err := sqlmock.New()
		assert.Nil(t, err)
		...
		got, err := mysqlbase.GetMGRGTID(db)
		assert.Nil(t, err)
		assert.Equal(t, want, *got)
	}
}
...
func TestMysqlBaseChangeToMasterForSemiSync(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.Nil(t, err)
	mysqlbase.SetQueryTimeout(10000)
	defer db.Close()
...
func TestMysqlBaseMGRChangeMasterToCommand(t *testing.T) {
	db, _, err := sqlmock.New()
	assert.Nil(t, err)
	mysqlbase.SetQueryTimeout(10000)
	defer db.Close()
  ...
  MASTER_PASSWORD = 'password' FOR CHANNEL 'group_replication_recovery'`}
	...
	got := mysqlbase.MGRChangeMasterToCommands(&master)
	assert.Equal(t, want, got)
}

func TestMysqlBaseMGRChangeMasterTo(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.Nil(t, err)
	mysqlbase.SetQueryTimeout(10000)
	defer db.Close()
	...
	err = mysqlbase.MGRChangeMasterTo(db, &master)
	assert.Nil(t, err)
}

func TestMysqlBaseChangeToMasterForMGR(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.Nil(t, err)
	mysqlbase.SetQueryTimeout(10000)
...
func TestMysqlBaseResetSlave(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.Nil(t, err)
	mysqlbase.SetQueryTimeout(10000)
	defer db.Close()
...
func TestCreateUserError(t *testing.T) {
	...
	// The sqlmock v1.5.0 uses the leftmost matching principle, so must test ssl=YES first.
	querys := []string{
...
func TestCreateReplUserWithoutBinlog(t *testing.T) {
	...
	mock.ExpectExec(queryList[5]).WillReturnResult(sqlmock.NewResult(1, 1))
