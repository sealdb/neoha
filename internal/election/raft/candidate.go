...
func (r *Candidate) processRequestVoteResponse(voteGranted *int, rsp *model.RaftRPCResponse, switchMaster *bool) {
	...
	case model.ErrorMySQLDown:
		...
		return
	case model.ErrorMGRAlreadyRunningOK:
		r.WARNING("get.vote.response.from[N:%v, V:%v].deny[ErrorMGRAlreadyRunningOK].downgrade.to.follower", rsp.GetFrom(), rsp.GetViewID())
		r.degradeToFollower()
		return
	default:
