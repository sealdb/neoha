...
func NewRaft(conf *config.Config, log *nlog.Log, db *database.Database,
	dbType database.DBType, state State) *Raft {
	r := &Raft{
		id:                       conf.Endpoint,
		conf:                     conf.Election.Raft,
		log:                      log,
...
func (r *Raft) updateEpoch(epochid uint64, peers []string, idlePeers []string) {
	...
	r.meta.Peers = peers
	r.meta.Peers = sanitizePeerList(r.meta.Peers)
	...
	r.meta.IdlePeers = sanitizePeerList(r.meta.IdlePeers)
...
// or when rejecting a stale RequestVote. It includes replicationPromotable plus
// MGR primary UUID checks at the Raft layer.
func (r *Raft) Promotable() bool {
	if !r.replicationPromotable() {
		return false
	}
	return r.mgrPrimaryPromotable()
}

func (r *Raft) mgrPrimaryPromotable() bool {
	if r.mysqlReplMode != model.ReplModeMGR {
		return true
	}
	myuuid, err := r.getUUID()
	if err != nil || myuuid == "" {
		r.ERROR("can.not.promotable.getUUID[%v].error:%v", myuuid, err)
		return false
	}
	if ok, puuid := r.isMGRClusterStatusOK(); ok && puuid != "" && puuid != myuuid {
		r.ERROR("can.not.promotable.mgr.cluster.status.is.ok[ok:%v, primaryuuid:%v, myuuid: %v]", ok, puuid, myuuid)
		return false
	}
	return true
}
