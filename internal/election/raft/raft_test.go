...
// 1. Start 3 rafts
// 2. check quorums
func testRaftQuorums(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
	raft := rafts[0]
...
func TestRaftQuorums_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftQuorums)
}
...
func testRaftClusterOnly1Node(t *testing.T, replMode model.MysqlReplMode) {
	var want, got State

	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 1, -1, replMode, false)
	defer cleanup()
...
func TestRaftClusterOnly1Node_MySQL_SemiSync(t *testing.T) {
	testRaftClusterOnly1Node(t, model.ReplModeSemiSync)
}
...
func testRaftLeaderDown(t *testing.T, replMode model.MysqlReplMode) {
	var want, got State
	var whoisleader int
	var leader *Raft
	testName := t.Name()
...
func TestRaftLeaderDown_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderDown)
}
...
func testRaftLeaderLocalCommit(t *testing.T, replMode model.MysqlReplMode) {
	var want, got State
	var whoisleader int
	var leader *Raft
...

	{
		...
		if replMode == model.ReplModeSemiSync {
			...
			leader.mysql.SetMysqlHandler(mysql.NewMockGTIDByNodes(3, -1, 3))
		}
		leader.Start()
...
func TestRaftLeaderLocalCommit_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderLocalCommit)
}
...
func testRaftDoubleClusterDiffraction(t *testing.T, replMode model.MysqlReplMode) {
	...
	// cluster1
	port := common.RandomPort(8000, 9000)
	_, rafts1, cleanup1 := MockRafts(log, port, 2, -1, replMode, false)
	defer cleanup1()

	// cluster2
	port = common.RandomPort(8000, 9000)
	_, rafts2, cleanup2 := MockRafts(log, port, 2, -1, replMode, false)
	defer cleanup2()
	...
	// 2. check cluster1
	{
		MockWaitLeaderEggs(rafts1, 1, replMode, false, -1)
		got = 0
	...
	// 2. check cluster2
	{
		MockWaitLeaderEggs(rafts2, 1, replMode, false, -1)
		got = 0
		want = (LEADER + FOLLOWER)
	...
	{
		MockWaitSomeElectionTimeout(rafts1, 2)
		if replMode == model.ReplModeMGR {
			MockUseSimpleMGRHandlers(rafts1)
		}
	...
	// 6. check cluster1
	{
		...
		isXn := replMode == model.ReplModeMGR
		MockWaitLeaderEggs(rafts1, 1, replMode, isXn, -1)
		got = 0
		want = (LEADER + STOPPED)
		for _, raft := range rafts1 {
			got += raft.getState()
	...
	// 7. check cluster2
	{
		MockWaitSomeElectionTimeout(rafts2, 2)
		got = 0
		want = (FOLLOWER + STOPPED)
		for _, raft := range rafts2 {
			got += raft.getState()
...
func TestRaftDoubleClusterDiffraction_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, func(t *testing.T, replMode model.MysqlReplMode) {
		if replMode == model.ReplModeMGR {
			// MGR leader loop requires mgrCnt >= quorum; a 2-node cluster with one
			// member stopped cannot keep LEADER (unlike semi-sync in this mock setup).
			t.Skip("MGR join quorum not satisfiable with one live node in a 2-member cluster")
		}
		testRaftDoubleClusterDiffraction(t, replMode)
	})
}
...
func testRaftLeaderDownAndUp(t *testing.T, replMode model.MysqlReplMode) {
	...
	// 6.  old-leader reset handlers to work
	{
		...
		if replMode == model.ReplModeMGR {
			MockWaitSomeElectionTimeout(rafts, 6)
		} else {
			MockWaitSomeElectionTimeout(rafts, 2)
		}
	}
...
func TestRaftLeaderDownAndUp_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderDownAndUp)
}
...
func testRaftEpochChange(t *testing.T, replMode model.MysqlReplMode) {
	...
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	cluster, rafts, cleanup := MockRafts(log, port, int(raftnums), -1, replMode, false)
	defer cleanup()
	...
	{
		exists = MockWaitPeerListCount(rafts, newpeer, len(cluster))
		assert.Equal(t, len(cluster), exists)
	...
	{
		exists = MockWaitPeerListCount(rafts, newpeer, 1)
		assert.Equal(t, 1, exists)
	}
}

func TestRaftEpochChange_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftEpochChange)
}
...
func testRaftEpochChangeUnderIDLE(t *testing.T, replMode model.MysqlReplMode) {
	var testName = "TestRaftEpochChange"
	var want, got, raftnums State
	var newpeer = "127.0.0.1:18888"
	var exists int
...
func TestRaftEpochChangeUnderIDLE_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftEpochChangeUnderIDLE)
}
...
func testRaftElectionUnderIDLEInMajority(t *testing.T, replMode model.MysqlReplMode) {
	var want, got, raftnums State
	var whoisleader int
	var leader string
	...
	port := common.RandomPort(8100, 8200)
	_, rafts, cleanup := MockRafts(log, port, int(raftnums), int(raftnums/2), replMode, false)
	defer cleanup()
	...
	{
		MockSetMysqlHandler(rafts[0], mysql.NewMockGTIDA(replMode))
...
func TestRaftElectionUnderIDLEInMajority_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftElectionUnderIDLEInMajority)
}
...
// 2. check the IDLE
func testRaftStartAsIDLE(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	conf := config.DefaultConfig()
	conf.Election.Raft.SuperIDLE = true
...
func TestRaftStartAsIDLE_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftStartAsIDLE)
}
...
func testRaftStartAsFOLLOWER(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8100, 8200)
	_, rafts, cleanup := MockRafts(log, port, 1, -1, replMode, false)
	defer cleanup()
...
func TestRaftStartAsFOLLOWER_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftStartAsFOLLOWER)
}
...
func testRaftInitRoleIsLeader(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	var newpeer = "127.0.0.1:18888"

	port := common.RandomPort(8100, 8200)
	_, rafts, cleanup := MockRafts(log, port, 2, -1, replMode, false)
	defer cleanup()
...
func TestRaftInitRoleIsLeader_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftInitRoleIsLeader)
}
...
func testRaftInitRoleIsFollower(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	var newpeer = "127.0.0.1:18888"
	var whoisleader = -1
...
func TestRaftInitRoleIsFollower_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftInitRoleIsFollower)
}
...
func testRaftInitRoleIsIdle(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	var whoisleader = -1
	port := common.RandomPort(8100, 8200)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
...
func TestRaftInitRoleIsIdle_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftInitRoleIsIdle)
}
...
func testRaft11Rafts1Cluster(t *testing.T, replMode model.MysqlReplMode) {
	var raftnums State
	var whoisleader int
	var leader *Raft
	...
	_, rafts, cleanup := MockRafts(log, port, int(raftnums), -1, replMode, false)
	defer cleanup()
...
func TestRaft11Rafts1Cluster_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaft11Rafts1Cluster)
}
...
func testRaftLeaderWithGTID(t *testing.T, replMode model.MysqlReplMode) {
	if replMode == model.ReplModeMGR {
		t.Skip("GTID leader election uses semi-sync mock GTIDs; see FailGTID MGR RPC tests")
	}
	var whoisleader int
	...
	// 5. wait rafts[1] elected as new leader
	{
		MockWaitLeaderEggs(rafts, 1, replMode, isMGR, GTIDCIDX)

		want := (LEADER + FOLLOWER + STOPPED)
		assert.True(t, MockWaitUntil(20*time.Second, 50*time.Millisecond, func() bool {
			got := State(0)
			whoisleader = -1
			for i, raft := range rafts {
				got += raft.getState()
			...
			return got == want && whoisleader == GTIDBIDX
		}), "expected node %d as leader after primary down", GTIDBIDX)
	}
}

func TestRaftLeaderWithGTID_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderWithGTID)
}
...
//  3. wait rafts[2] elected as leader
//  4. Stop all rafts
func testRaftWithFollowerGetSlaveGTIDError(t *testing.T, replMode model.MysqlReplMode) {
	var whoisleader int
...
func TestRaftWithFollowerGetSlaveGTIDError_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftWithFollowerGetSlaveGTIDError)
}
...
//  5. Stop all rafts
func testRaftLeaderPurgeBinlog(t *testing.T, replMode model.MysqlReplMode) {
	conf := config.DefaultConfig()
	...
	// leader
	{
		...
		assert.True(t, MockWaitUntil(15*time.Second, 50*time.Millisecond, func() bool {
			...
			return got == LEADER+FOLLOWER+FOLLOWER
		}))
		...
		assert.Equal(t, want, got)
		assert.True(t, MockWaitLeaderPurgeBinlogs(rafts[leaderIdx], 1, 5*time.Second))

		// disable purge binlog
		rafts[leaderIdx].SetSkipPurgeBinlog(true)
		purged := rafts[leaderIdx].stats.LeaderPurgeBinlogs

		assert.True(t, MockWaitUntil(5*time.Second, 50*time.Millisecond, func() bool {
			return rafts[leaderIdx].stats.LeaderPurgeBinlogs == purged
		}))

		// enable purge binlog
		rafts[leaderIdx].SetSkipPurgeBinlog(false)
		purged = rafts[leaderIdx].stats.LeaderPurgeBinlogs
...
func TestRaftLeaderPurgeBinlog_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderPurgeBinlog)
}
...
func TestRaftLeaderCheckSemiSync(t *testing.T) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, model.ReplModeSemiSync, false)
	defer cleanup()
	...
	// leader
	{
		...
		MockWaitSomeElectionTimeout(rafts, 16)
...
func testRaftChangeMasterToFail(t *testing.T, replMode model.MysqlReplMode) {
	if replMode == model.ReplModeMGR {
		t.Skip("MGR change-master-to failure path differs from semi-sync; covered by MGR integration tests")
	}
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
...
func TestRaftChangeMasterToFail_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftChangeMasterToFail)
}
...
// 1. Start 1 rafts
// 2. check leader
func testRaft1Nodes(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 1, -1, replMode, false)
	defer cleanup()
...
func TestRaft1Nodes_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaft1Nodes)
}
...
func testRaft2Nodes(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 2, -1, replMode, false)
	defer cleanup()
...
func TestRaft2Nodes_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaft2Nodes)
}
...
func testRaft2NodesWithGTID(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 2, -1, replMode, false)
	defer cleanup()
...
func TestRaft2NodesWithGTID_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaft2NodesWithGTID)
}

// TEST EFFECTS:
// test the leader heartbeat acks less than the quorum.
func testRaftLeaderAckLessThanQuorum(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
...
func TestRaftLeaderAckLessThanQuorum_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderAckLessThanQuorum)
}
...
func testRaftLeaderWaitUntilAfterGTIDError(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
...
func TestRaftLeaderWaitUntilAfterGTIDError_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderWaitUntilAfterGTIDError)
}
...
func testRaftLeaderChangeToMasterError(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
...
func TestRaftLeaderChangeToMasterError_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftLeaderChangeToMasterError)
}
...
func testRaftElectionUnderLearnerInMinority(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
...
func TestRaftElectionUnderLearnerInMinority_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftElectionUnderLearnerInMinority)
}
...
func testRaftElectionUnderFollowerAndCandidateAlternate(t *testing.T, replMode model.MysqlReplMode) {
	log := nlog.NewStdLog(nlog.Level(nlog.PANIC))
	port := common.RandomPort(8000, 9000)
	_, rafts, cleanup := MockRafts(log, port, 3, -1, replMode, false)
	defer cleanup()
...
func TestRaftElectionUnderFollowerAndCandidateAlternate_MySQL(t *testing.T) {
	forEachMySQLReplMode(t, testRaftElectionUnderFollowerAndCandidateAlternate)
}
